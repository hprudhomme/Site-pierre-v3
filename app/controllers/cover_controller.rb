class CoverController < ApplicationController
    ...
    def create
        ...
        @cover = Cover.new(title: @params_cover["title"], content: @params_cover["content"], background: @params_cover["background"])
    ...
    def update     
      ...
      @cover = Cover.find(params[:id])
      if @cover.update(title: params[:title], content: params[:content])
          redirect_to request.referrer;
      else
        redirect_to request.referrer;
      end
